...
import { generateWholesaleProductLedger } from '../../../utils/wholesaleInventoryLedger';
import { api } from '../../../services/api';
import { format } from 'date-fns';
import { exportToCSV } from '../../../utils/export';
import type { WsProduct } from '../../../types';

interface WholesaleProductLedgerDialogProps {
...
export function WholesaleProductLedgerDialog({ product, onClose }: WholesaleProductLedgerDialogProps) {
    const [daysBack, setDaysBack] = useState<DateRange>('7');

    const { data, isLoading } = useQuery({
        queryKey: ['wsProductLedger', product?.id, daysBack, product?.stock],
        queryFn: async () => {
            if (!product) return null;
            // Wholesale audit logs
            const auditLogs = await api.getAuditLogsByProduct(product.id);
            return await generateWholesaleProductLedger(product, parseInt(daysBack), auditLogs);
        },
        enabled: !!product
    });
    ...
    const handleExportCSV = () => {
        ...
        const csvData = data.rows.map((row) => ({
            ...
            'Cost Price (Nrs)': row.wacc.toFixed(2)
        }));
        exportToCSV(csvData, `Wholesale_${product.name}_ledger_${format(new Date(), 'yyyy-MM-dd')}`);
    };
            ...
            <DialogContent className="w-[95vw] md:max-w-5xl max-h-[90vh] overflow-hidden flex flex-col bg-slate-50 p-0">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1 flex items-center gap-3">
                            ...
                            <span className="flex items-center gap-1">
                                ...
                                <span className="font-bold text-slate-900">Nrs. {product.cost_price?.toFixed(2) || '—'}</span>
                            </span>
                        </p>
                    </div>
                ...
                {(isLoading || !data) ? (
                    <div className="flex flex-col items-center justify-center p-12 flex-1">
                        ...
                        <p className="text-slate-500 font-medium">Crunching daily wholesale ledger...</p>
                    </div>
                ) : (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-5">
                            ...
                            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                ...
                                <div className="flex items-center gap-2">
                                    <p className={`text-base font-bold ${data.analytics.profit >= 0 ? 'text-sky-700' : 'text-red-600'}`}>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {data.rows.map((row, i) => {
                                            ...
                                            return (
                                                <TableRow key={i} className={`hover:bg-slate-50/50 ${isToday ? 'bg-sky-50/30' : ''}`}>
                                                    <TableCell className="text-xs text-slate-700 py-2 font-medium align-top">
                                                        {format(row.date, 'MMM d, yy')}
                                                        {isToday && <span className="ml-1 text-[9px] text-sky-500 font-bold block">TODAY</span>}
                                                    </TableCell>
                                                    ...
                                                    {/* Net Profit & Margin */}
                                                    <TableCell className="text-right py-2 align-top">
                                                        <div className={`text-xs font-bold ${row.dayProfit > 0 ? 'text-sky-700' : row.dayProfit < 0 ? 'text-red-600' : 'text-slate-400'}`}>
                                        ...
                                        <TableRow className="bg-slate-50 border-t-2 border-slate-200 font-bold">
                                            ...
                                            {/* Profit & Margin Total */}
                                            <TableCell className="text-right py-2 align-top">
                                                <div className={`text-xs font-black ${data.analytics.profit >= 0 ? 'text-sky-700' : 'text-red-600'}`}>
                                                    Nrs. {data.analytics.profit.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                                                </div>
