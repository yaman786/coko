...
async function run() {
  ...
  const { data, error } = await supabase
    ...
    .eq('category', 'Bio-products')
    .eq('isDeleted', false)
    .order('name');

  if (error) {
    console.error('Error fetching Bio-products:', error);
  } else {
    console.log(`Found ${data.length} active Bio-products.`);
    data.forEach(d => console.log(`- ${d.name} | Subcategory: ${d.subcategory || 'Empty'}`));
  }
}
run();
